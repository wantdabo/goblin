...
public sealed class Stage
{
    ...
    /// 预制创建器集合
    /// </summary>
    public GBLDict<Type, Prefab> prefabs { get; set; }
    /// <summary>
    /// 投影系统
    /// </summary>
    ...
    private void Prefabs()
    {
        ...
        prefabs = ObjectCache.Ensure<GBLDict<Type, Prefab>>();
    ...
    {
        // 检查 Behavior 容器是否存在
        if (false == cache.behaviors.TryGetValue(type, out var list)) cache.behaviors.Add(type, list = ObjectCache.Ensure<GBLList<Behavior>>());
        if (false == cache.behaviordict.TryGetValue(id, out var dict)) cache.behaviordict.Add(id, dict = ObjectCache.Ensure<GBLDict<Type, Behavior>>());
    ...
    public T AddBehaviorInfo<T>(ulong id) where T : BehaviorInfo, new()
    {
        ...
        if (false == cache.behaviorinfodict.TryGetValue(id, out var dict)) cache.behaviorinfodict.Add(id, dict = ObjectCache.Ensure<GBLDict<Type, BehaviorInfo>>());
    ...
    public sealed class StageCache
    {
        ...
        /// </summary>
        public GBLDict<ulong, GBLDict<Type, Behavior>> behaviordict { get; set; }
        /// <summary>
        /// 行为列表, 键为行为类型, 值为该行为类型的所有 Behavior 列表
        /// </summary>
        public GBLDict<Type, GBLList<Behavior>> behaviors { get; set; }
        /// <summary>
        ...
        /// Rmv Behavior列表
        /// </summary>
        public GBLList<Behavior> rmvbehaviors { get; set; }
        /// <summary>
        /// Rmv BehaviorInfo 列表
        /// </summary>
        ...
        public void Dispose()
        {
            behaviordict.Dispose();
            behaviors.Dispose();
            behaviorinfodict.Dispose();
            rmvactorset.Dispose();
