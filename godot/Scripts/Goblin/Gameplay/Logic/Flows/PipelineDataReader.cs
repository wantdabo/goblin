...
public static class PipelineDataReader
{
    ...
    /// </summary>
    private static GBLDict<uint, PipelineData> datas { get; set; } = new();
    /// <summary>
    /// 脚本数据字典, 键为脚本 ID, 值为脚本处理器
    /// </summary>
    private static GBLDict<uint, Scripting> scriptings { get; set; } = new();
    ...
    public static void Format(this PipelineData data)
    {
        ...
        data.length = length;
        data.instructs.Sort((a, b) =>
        {
            if (a.begin == b.begin) return a.end.CompareTo(b.end);
