...
public class LoginProxy : Proxy<LoginModel>
{
    ...
    private void OnS2CLogin(S2CLoginMsg msg)
    {
        ...
        else if (2 == msg.code) engine.eventor.Tell(new MessageBlowEvent { type = 2, desc = "用户未注册." });
        else if (3 == msg.code) engine.eventor.Tell(new MessageBlowEvent { type = 2, desc = "用户密码错误." });
    }
    ...
    private void OnS2CLogout(S2CLogoutMsg msg)
    {
        if (1 == msg.code)
        {
            ...
            data.uuid = null; data.signined = false;
            engine.gameui.QuickClose();
        }
        else if (2 == msg.code) engine.eventor.Tell(new MessageBlowEvent { type = 2, desc = "用户未登录." });
        else if (3 == msg.code)
        {
    ...
    {
        if (1 == msg.code) engine.eventor.Tell(new MessageBlowEvent { type = 1, desc = "注册成功." });
        else if (2 == msg.code) engine.eventor.Tell(new MessageBlowEvent { type = 2, desc = "用户已存在." });
    }

    private void OnS2CRoleJoined(S2CRoleJoinedMsg msg) => eventor.Tell<RoleJoinedEvent>();
