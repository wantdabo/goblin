...
namespace Goblin.Gameplay.Render.Resolvers
{
    public class Spatial : Resolver
    {
        ...
        {
            var statebundles = world.summary.GetStateBundles(RIL_DEFINE.SPATIAL);
            if (null == statebundles) return;
            foreach (var state in statebundles)
            {
                var node = world.EnsureAgent<Node>(state.actor);
                var spatial = (RIL_SPATIAL)state.ril;
